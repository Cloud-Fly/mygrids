...
//			
                ...
                ($.inArray('asc', oCol.asSorting) == -1 && $.inArray('desc', oCol.asSorting) == -1) )
      {
		...
		function _fnAjaxParameters( oSettings )
		{
			...
			aoData = {
				columnNum : iColumns,
				columns : _fnColumnOrdering(oSettings),
				regex : oSettings.oPreviousSearch.bRegex,
				echo : oSettings.iDraw,
				search : oSettings.oPreviousSearch.sSearch,
				displayStart : oSettings._iDisplayStart,
				displayLength : oSettings.oFeatures.bPaginate !== false ? oSettings._iDisplayLength : -1
			};
			aoData.columnInfo = [];
			aoData.sortInfo = [];
			aoData.filterInfo = [];
			for (i = 0; i < iColumns; i++) {
				...
				var cInfo = {};
				cInfo.name = typeof (mDataProp) == "function" ? 'function' : mDataProp;
				cInfo.regex = oSettings.aoPreSearchCols[i].bRegex;
				cInfo.searchable = oSettings.aoColumns[i].bSearchable;
				cInfo.sortable = oSettings.aoColumns[i].bSortable;
				aoData.columnInfo.push(cInfo);

				// filter
				if (oSettings.aoPreSearchCols[i].sSearch) {
					var fInfo = {};
					fInfo.name = typeof (mDataProp) == "function" ? 'function' : mDataProp;
					fInfo.search = oSettings.aoPreSearchCols[i].sSearch;
					aoData.filterInfo.push(fInfo);
				}
			}
			...
			if ( oSettings.oFeatures.bSort !== false )
			{
				...
				aoData.iSortingCols= iFixed+iUser;
				for ( i=0 ; i<iFixed ; i++ )
				{
					var sInfo = {};
					var j=oSettings.aaSortingFixed[i][0];
					mDataProp = oSettings.aoColumns[j].mDataProp;
					
					sInfo.columnId = typeof (mDataProp) == "function" ? 'function' : mDataProp;
					sInfo.sortOrder=oSettings.aaSortingFixed[i][1];
					aoData.sortInfo.push(sInfo);
				}
				
				for ( i=0 ; i<iUser ; i++ )
				{
					var sInfo = {};
					var j=oSettings.aaSorting[i][0];
					mDataProp = oSettings.aoColumns[j].mDataProp;
					sInfo.columnId = typeof (mDataProp) == "function" ? 'function' : mDataProp;
					sInfo.sortOrder=oSettings.aaSorting[i][1];
					aoData.sortInfo.push(sInfo);
				}
