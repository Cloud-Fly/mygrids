...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			_index : {},
			afterInsertRow: null,
			ajaxGridOptions :{},//对ajax参数进行全局设置，可以覆盖ajax事件：error，complete 和 beforeSend
			altclass : 'ui-priority-secondary',//用于表格行交替变色的class属性。你可以定义自己的class来替代默认值。只有当altRows属性设置为true时，该属性才起作用
			altRows: false,//设置表格是否允许行交替变色值。樣式以預設值altclass設定
			...
			cellLayout: 5,//定义了单元格padding + border 宽度。通常不必修改此值。初始值为5，paddingLef?2+paddingRight?2+borderLeft?1=5
			...
			data : [],
			datatype: "xml",//从服务器端返回的数据类型，（表格期望接收的数据类型）。可选类型：xml，xmlstring，json，local，function
			deselectAfterSort : true,//只有当datatype为local时起作用。当排序时不选择当前行
			direction : "ltr",//表格中文字的显示方向，从左向右（ltr）或者从右向左（rtr）。
			editurl: null,//定义对form编辑时的url
			ExpandColumn: null,//指定那列来展开tree grid，默认为第一列，只有在treeGrid为true时起作用
			...
			mtype: "GET",//定义使用哪种方法发起请求，GET或者POST
			multiboxonly : false,//只有当multiselect = true.起作用，当multiboxonly 为ture时只有选择checkbox才会起作用，
			multikey: false,//只有在multiselect设置为ture时起作用，定义使用那个key来做多选。shiftKey，altKey，ctrlKey
			multiselect: false,//显示checkbox
			...
			onSortCol: null,
			page: 1,//设置初始的页码
			pager: "",//指定分页栏对象，必须为一个有效的html元素。可以是'pager', '#pager', jQuery('#pager').推荐用'#pager'
			...
			rowNum: 20,//用于设置Grid中一次显示的行数，默认值为20。正是这个选项将参数rows（prmNames中设置的）通过url选项设置的链接传递到Server。注意如果Server返回的数据行数超过了rowNum的设定，则Grid也只显示rowNum设定的行数。
			...
			subGridWidth: 20,
			toolbar: [false,""],//表 格的工具栏。数组中有两个值，第一个为是否启用，第二个指定工具栏位置（相对于body layer），如：[true,”both”] 。工具栏位置可选值：“top”,”bottom”, “both”. 如果工具栏在上面，则工具栏id为“t_”+表格id；如果在下面则为 “tb_”+表格id；如果只有一个工具栏则为 “t_”+表格id
			toppager: false,
			tree_root_level : 0,//root元素的级别
			treeANode : -1,
			treeGrid : false,//启用或者禁用treegrid模式
			treeGridModel : 'nested',//treeGrid所使用的方法
			treeReader : {},
			userData: {},//从request中取得的一些用户信息
			userDataOnFooter : false,//当为true时把userData放到底部，用法：如果userData的值与colModel的值相同，那么此列就显示正确的值，如果不等那么此列就为空,主要用作统计
			viewrecords: false,//是否显示目前记录编号与总笔数
			viewsortcols : [false,'vertical',true],//定 义排序列的外观跟行为。数据格式：[false,'vertical',true].第一个参数是说，是否都要显示排序列的图标，false就是只显示当 前排序列的图标；第二个参数是指图标如何显示，vertical：排序图标垂直放置，horizontal：排序图标水平放置；第三个参数指单击功 能，true：单击列可排序，false：单击图标排序。说明：如果第三个参数为false则第一个参数必须为ture否则不能排序
			//width:500,//如果设置则按此设置为主，如果没有设置则按colModel中定义的宽度计算
			xmlReader: {},
