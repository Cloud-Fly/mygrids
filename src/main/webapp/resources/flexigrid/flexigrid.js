...
(function($) {
	...
	$.addFlex = function(t, p) {
		...
		// 引用默认属性
		p = $.extend({
			timeout : 1000 * 60 * 2,// 设置获取数据超时时间，2分钟
			addparams : [], // 查询时带上的参数,$.ajax({ 这个前面一行。//add cx
			height : 200, // flexigrid插件的高度，单位为px
			width : 'auto', // 宽度值，auto表示根据每列的宽度自动计算
			striped : true, // 是否显示斑纹效果，默认是奇偶交互的形式
			novstripe : true,// 是否在表格内隐藏调整列宽的那条线
			minwidth : 30,// 列的最小宽度
			minheight : 80, // 列的最小高度
			JQueryUI:true,//是否使用jqueryui的css
			resizable : false, // 是否可伸缩
			url : false, // ajax方式对应的url地址
			method : 'POST', // 数据发送方式
			dataType : 'json', // 数据加载的类型
			rowId : false, // 行的ID
			showcheckbox : false, // 是否要多选框
			errormsg : '连接错误!', // 错误提示信息
			usepager : true, // 是否显示分页工具条
			nowrap : true, // 是否不换行
			titleAlign:"center",//标题的css，默认居中
			page : 1, // 默认当前页
			total : 1, // 总页面数
			useRp : true, // 是否可以动态设置每页显示的结果数
			rp : 10, // 每页默认的结果数
			rpOptions : [ 5, 10, 30, 50, 100], // 可选择设定的每页结果数
			title : false, // 标题名称
			...
			showToggleBtn : true, // 是否允许显示标题栏的隐藏列信息的按钮
			hideOnSubmit : true, // ？
			showTableToggleBtn : true, // 显示隐藏Grid的按钮，在title右上方
			singleSelect : false, // 只能选中一行
			autoload : true, // 自动加载
			blockOpacity : 0.5, // 透明度设置
			preProcess : false,// 预处理数据 preProcess(data);
			onDragCol : false,// 重新绘制？
			onToggleCol : false, // 当在行之间转换时，可在此方法中重写默认实现，基本无用
			onChangeSort : false, // 当改变排序时，可在此方法中重写默认实现，自行实现客户端排序
			onSuccess : false, // 成功后执行 调用自定义的计算函数
			onSubmit : false,
			onError : false, // 出错时调用的函数
			rowHandler : false, // 可以绑定一行的事件
			onRowChecked : false
		// check点击时触发的事件
		}, p);
		...
		// set gDiv
		if(p.JQueryUI){
			g.gDiv.className = 'flexigrid  ui-widget ui-widget-content ui-corner-all';
			} else{
		...
		if (p.buttons) {
			if(p.JQueryUI){
			g.tDiv.className = 'tDiv ui-state-default';
			} else{
			...
			for ( var i = 0; i < p.buttons.length; i++) {
				...
				if (!btn.separator) {
					...
					$(tDiv2).append("<div class='btnseparator ui-state-default '></div>");
				}
			}
			$(g.tDiv).append(tDiv2);
		...
		// set hDiv
		if(p.JQueryUI){
			g.hDiv.className = 'hDiv ui-state-default';
			} else{
		...
		// set bDiv
		if(p.JQueryUI){
			g.bDiv.className = 'bDiv ui-state-default';
			}else{
		...
		if (p.resizable && p.height != 'auto') {
			if(p.JQueryUI){
				g.vDiv.className = 'vGrip ui-state-default';
				}else{
		...
		if (p.usepager) {
			if(p.JQueryUI){
				g.pDiv.className = 'pDiv ui-state-default';
				}else{
					g.pDiv.className = 'pDiv';
				}
			g.pDiv.innerHTML = '<div class="pDiv2"></div>';
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton" title="首页"><span></span></div><div class="pPrev pButton" title="上一页"><span></span></div> </div> <div class="btnseparator  ui-state-default "></div> <div class="pGroup"><span class="pcontrol">第 <input type="text" size="4" value="1" />页,共 <span> 1 </span>页</span></div> <div class="btnseparator ui-state-default "></div> <div class="pGroup"> <div class="pNext pButton" title="下一页"><span></span></div><div class="pLast pButton" title="尾页"><span></span></div> </div> <div class="btnseparator ui-state-default "></div> <div class="pGroup"> <div class="pReload pButton"  title="刷新"><span></span></div> </div> <div class="btnseparator ui-state-default "></div> <div class="pGroup"><span class="pPageStat"></span></div>';
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'><select name='rp'>" + opt + "</select></div> <div class='btnseparator ui-state-default '></div>");
			...
			// add search button
			if (p.searchitems) {
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'> <div class='pSearch pButton' title='快速查找'><span></span></div> </div>  <div class='btnseparator ui-state-default '></div>");
				...
				// add search box
				if(p.JQueryUI){
					g.sDiv.className = 'sDiv ui-state-default';
				...
				$(g.sDiv).append(
						"<div class='sDiv2'>快速查找：<input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>" + sopt
								+ "</select> <input type='button' name='find' value='查找'  class='ui-state-hover'/> <input type='button' name='clear' value='清除' class='ui-state-hover'/></div>");
		...
		if (p.title) {
			if(p.JQueryUI){
				g.mDiv.className = 'mDiv ui-widget-header ui-corner-top ui-helper-clearfix';
			}else{
				g.mDiv.className = 'mDiv';
			}
			if(p.titleAlign){
			g.mDiv.innerHTML = '<div style="text-align: '+p.titleAlign+';">' + p.title + '</div>';
			}else{
				g.mDiv.innerHTML = '<div style="text-align:center;">' + p.title + '</div>';
			}
			$(g.gDiv).prepend(g.mDiv);
			if (p.showTableToggleBtn) {
				
				if(p.JQueryUI){
				$(g.mDiv).append('<div class="ptogtitle" title="最小化/最大化表格"><span class="ui-icon ui-icon-circle-triangle-n"></span></div>');
				}else{
				...
				$('div.ptogtitle', g.mDiv).click(function() {
					...
					if(p.JQueryUI){
						if($('div.ptogtitle span', g.mDiv).hasClass("ui-icon-circle-triangle-n")){
							$('div.ptogtitle span', g.mDiv).removeClass("ui-icon-circle-triangle-n").toggleClass('ui-icon-circle-triangle-s');
						}else{
							$('div.ptogtitle span', g.mDiv).removeClass("ui-icon-circle-triangle-s").toggleClass('ui-icon-circle-triangle-n');
						}
					}else {$(this).toggleClass('vsble');  }
				});
			}
			// g.rePosDrag();
		}
		...
		if ($('th', g.hDiv).length) {
			if(p.JQueryUI){
			g.nDiv.className = 'nDiv ui-state-default';
			}else{
			...
			if(p.JQueryUI){
				$(g.nBtn).addClass('nBtn ui-state-hover');
				}else{
					$(g.nBtn).addClass('nBtn');
				}
//			$(g.nBtn).hover(function() {
//				$(this).addClass('ui-state-hover');
//			}, function() {
//				$(this).removeClass('ui-state-hover');
//			});
			$(g.nBtn).html('<div></div>').attr('title', '隐藏/显示列').click(function() {
				$(g.nDiv).toggle();
				return true;
			});
